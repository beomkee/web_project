...
<%
	...
	LeftMenuDBMysql db = LeftMenuDBMysql.getInstance();
	List<LeftMenuDataBean> left = db.getLeftMenu(division);
%>
	...
	<div class="sidebar-inner">
		...
		<ul class="navi">
			<li class="nred current">
				<a href="<%= request.getContextPath() %>/metro/contentJSP/main_<%=div%>.jsp">
						...
						<c:if test="${i.collapse eq 'n'}">
							<li class="has_submenu ngray">
								...
								<c:if test="${i.num == j.parent_num}">
									<ul>
										<li>
											<a href="<%=request.getContextPath()%>/metro/${j.url}">${j.text}</a>
										</li>
									</ul>
								</c:if>
								</c:forEach>
